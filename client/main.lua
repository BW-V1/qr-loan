...
RegisterNetEvent('qr-loan:ClientNotify')
AddEventHandler('qr-loan:ClientNotify',function(Text, TextType)
    QRCore.Functions.Notify(Text, TextType, 5000)
end)

RegisterNetEvent('qr-loan:ConfirmYourLoan')
AddEventHandler('qr-loan:ConfirmYourLoan',function(Judge, Loan)
...
function CrateLoanMenu(PlyId)
    QRCore.Functions.TriggerCallback('qr-loan:DoesPlayerHaveLoan', function(DoesPlayerHaveLoan)
        ...
        elseif DoesPlayerHaveLoan == false then
            local input = lib.inputDialog(Config.text['CrateLoanMenu'].input2, {
                { type = 'select', label = Config.text['CrateLoanMenu'].labels, options = Config.LoansMenu},
                { type = "checkbox", label = Config.text['CrateLoanMenu'].labels2, checked = false }
            })
            if input and input[2] and input[2] == true then
                local Loan
                if input[1] then
